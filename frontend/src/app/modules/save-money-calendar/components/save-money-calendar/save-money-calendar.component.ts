...
export class SaveMoneyCalendarComponent implements OnInit {
  ...
  public usedValues: number[] = this.bdData
    .map((item) => item.map((day) => day.amount))
    .flat();
  ...
  public readonly data = computed<CalendarMonthData>(() =>
    this.getData(this.displayedMonth())
  );
  ...
  private getData(month: number): CalendarMonthData {
    ...
    const newMonthData = Array.apply(null, Array(this.monthDays())).map(
      (_value, index) => {
        const isRandom = (index + 1) % 3 === 0;
        const dayInYear = DateHelper.daysIntoYear(
          new Date(this.today.getFullYear(), month - 1, index + 1)
        );
